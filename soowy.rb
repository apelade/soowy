...
OptionParser.new do |opts|
  ...
  opts.banner = """
  Usage:
  ruby soowy.rb -v -r regex -f file,file..
  or pipe command:
  find path/to/scss -type f -name test\\*.txt | xargs ruby soowy.rb -r regex -f
  """
  opts.on("-r", "--regex /my_regex/", "regex used to match candidates") do |regex|
    options[:regex] << regex
  end
